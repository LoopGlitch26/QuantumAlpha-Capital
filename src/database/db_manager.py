"""
Database Manager for QuantumAlpha Capital.
...
class DatabaseManager:
    ...
    def __init__(self, db_url: str = None):
        ...
        Args:
            db_url: SQLAlchemy database URL. Defaults to sqlite:///data/quantumalpha.db
        """
        if db_url is None:
            # Default to SQLite in data/ directory
            db_path = os.path.join('data', 'quantumalpha.db')
    ...
    def save_agent_state(
        ...
        with self.session_scope() as session:
            ...
            logger.debug(f"Saved agent state: balance={balance:.2f}, total_value={total_value:.2f}")
            return state

    def get_latest_agent_state(self) -> Optional[BotState]:
