"""
Agent Service - Manages agent lifecycle and provides data access for GUI
"""
...
from src.backend.bot_engine import QuantumMarketProcessor, SystemState
from src.backend.config_loader import CONFIG


class AgentService:
    """Service layer for agent management and data access"""

    def __init__(self):
        self.market_processor: Optional[QuantumMarketProcessor] = None
    ...
    async def start(self, assets: Optional[List[str]] = None, interval: Optional[str] = None):
        ...
        """
        if self.market_processor and self.market_processor.processing_active:
        ...
        try:
            # Create market processor with callbacks
            self.market_processor = QuantumMarketProcessor(
                instruments=assets,
                timeframe=interval,
                on_system_update=self._on_system_update,
                on_execution_complete=self._on_execution_complete,
                on_system_alert=self._on_system_alert
            )

            # Start the agent
            await self.market_processor.initialize_processing()

            self.logger.info(f"Agent started successfully - Assets: {assets}, Interval: {interval}")
    ...
    async def stop(self):
        """Terminate the market processor"""
        if not self.market_processor:
            return

        try:
            await self.market_processor.terminate_processing()
            self.logger.info("Market processor terminated successfully")
        except Exception as e:
            self.logger.error(f"Error terminating processor: {e}", exc_info=True)
            raise

    def is_running(self) -> bool:
        """Check if agent is currently running"""
        return self.market_processor is not None and self.market_processor.processing_active

    def get_state(self) -> SystemState:
        """Get current system state"""
        if self.market_processor:
            return self.market_processor.get_system_state()
        return SystemState()
    ...
    async def close_position(self, asset: str) -> bool:
        ...
        """
        if not self.market_processor:
            self.logger.error("Market processor not initialized")
            return False

        try:
            success = await self.market_processor.close_position(asset)
            if success:
    ...
    def get_assets(self) -> List[str]:
        ...
        if self.market_processor:
            return self.market_processor.get_assets()
    ...
    async def test_api_connections(self) -> Dict[str, bool]:
        ...
        try:
            ...
            # Test OpenRouter (via neural engine)
            from src.backend.agent.decision_maker import QuantumDecisionEngine
            neural_engine = QuantumDecisionEngine()
            try:
    ...
    async def refresh_market_data(self) -> bool:
        """
        Manually refresh market data from Hyperliquid without starting the agent.
        ...
        try:
            ...
            # Update agent state with fresh data (create new state if agent not running)
            if not self.market_processor:
                # Create a temporary system state for display
                state = SystemState()
            else:
                state = self.market_processor.get_system_state()
    ...
    def approve_proposal(self, proposal_id: str) -> bool:
        ...
        Returns:
            True if approval was sent (async execution), False if agent not running
        """
        if not self.market_processor or not self.market_processor.processing_active:
            self.logger.error("Market processor not active - cannot approve proposal")
            return False

        try:
            # Schedule async execution
            asyncio.create_task(self.market_processor.approve_proposal(proposal_id))
    ...
    def reject_proposal(self, proposal_id: str, reason: str = "User rejected") -> bool:
        ...
        Returns:
            True if rejection was sent (async execution), False if agent not running
        """
        if not self.market_processor or not self.market_processor.processing_active:
            self.logger.error("Market processor not active - cannot reject proposal")
            return False

        try:
            # Schedule async execution
            asyncio.create_task(self.market_processor.reject_proposal(proposal_id, reason))
    ...
    def get_pending_proposals(self) -> List[Dict]:
        ...
        Returns:
            List of proposal dicts, or empty list if agent not running
        """
        if not self.market_processor:
            return []

        try:
            proposals = self.market_processor.get_pending_proposals()
    ...
    def _on_system_update(self, state: SystemState):
        """
        Callback when system state updates.
        ...
        self.equity_history.append({
            'time': state.last_sync or datetime.now(timezone.utc).isoformat(),
    ...
    def _on_execution_complete(self, execution: Dict):
        """
        Callback when execution is completed.
        Adds event to activity feed.
        """
        instrument = execution.get('asset', '')
        action = execution.get('action', '').upper()
        amount = execution.get('amount', 0)
        price = execution.get('price', 0)

        message = f"{action} {amount:.6f} {instrument} @ ${price:,.2f}"
        self._add_event(message)

    def _on_system_alert(self, alert: str):
        """
